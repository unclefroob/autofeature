...
- **A feature header is three things, and a design export gives you none of them.** The **gradient** is brand → `#00266E` (web `from-brand to-brand-800`, iOS `headerGradientTop/Bottom`, Android `HeaderTop/HeaderBottom`). The **navigation controls** are the shared glass ones (iOS `FloatingGlassButton`, Android `ChatIconButton`), never a hand-rolled button on a `white.opacity(0.15)` plate. The **extent** runs behind the status bar: iOS `.ignoresSafeArea(edges: .top)` with the header's own `.padding(.top, 56)`, Android `WindowInsets.statusBars` added to the header padding. Company Links shipped with all three wrong because it was built from the export's own chrome, and the extent was the most visible of them — a grey band above the blue with the clock sitting on grey. Copy an EXISTING feature's header (Chat is the reference), not the mock's CSS.
- **A missing modifier has no line in a diff.** The extent bug above was invisible to code review and to the compiler, and correct-looking in the patch; it took a simulator screenshot. Any change to a screen's chrome gets rendered and looked at before it is called done — reading the diff against a reference file is not the same check.
