...
- **`main` is a deploy trigger, and what it triggers differs per repo.** `ritchies-platform-api` auto-deploys to the dev App Service on merge; `ritchies-platform-web` auto-deploys the Static Web App; **`ritchies-mobile` starts an App Store Connect build** (Xcode Cloud, configured in App Store Connect and NOT in the repo, so an empty `.github/workflows` proves nothing — and it posts no status back to GitHub, so commits carry zero check-runs and `gh` can never tell you the result). `ritchies-android` has nothing automatic today.
