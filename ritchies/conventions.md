...
- **A flat overscroll cap forces a VERTICAL header gradient — on EVERY header, not just the capped ones.** iOS fills the area above a scrolling header with `headerOverscrollCap(_:)`, which paints ONE FLAT colour. A flat block can only match a top edge that is a single colour, so a `.topLeading → .bottomTrailing` gradient — whose top edge is a horizontal ramp already well toward `#00266E` on the right — meets it at a visible seam the moment anyone pulls down. Header gradients are therefore `startPoint: .top, endPoint: .bottom`, and the cap colour MUST be the gradient's own first stop (check it: Announcements, Calendar and the chat admin panel use their own colour pairs, not the tokens). **Fix every header, including the ones with no cap** — the drawer's profile header and three Calendar headers had no seam to show and were skipped twice for that reason, which left one colour pair painted both ways and no record that the direction was load-bearing. A cap added later brings the seam straight back. iOS-only: Android pins its headers and Compose overscroll is a stretch rather than a fill, and web has no cap.
- **A codebase-wide claim needs a check that could have failed.** "No header gradients are diagonal now" was stated on a grep requiring `topLeading` and `gradient` on the SAME line; the three that were left declare the colours on one line and the start point on the next, so that check could not have found them whatever the state of the code. Swift declarations wrap, so sweep with a multi-line matcher and classify each hit (a header carries `.padding(.top, 56)` and an `UnevenRoundedRectangle` bottom; an avatar or a card does not). The wider trap: a sweep scoped to "which screens show the bug" silently excludes every screen where the same mistake is latent.
- **A cancelled request is not a failure.** SwiftUI cancels a `.refreshable`'s task when the pull ends and a `.task` when its view goes away, killing the URLSession request underneath. That surfaces as `URLError.cancelled`, whose `localizedDescription` is the bare word "cancelled", so a catch that treats every error alike puts a full error screen up reading "Couldn't load that — cancelled" over data that is perfectly good. Every load path returns early on `Error.isCancellation` (`APIClient.swift`, which unwraps the URLError nested inside `APIError.transport`) and leaves state untouched. Where a store used `try?` the error never reaches the caller, so those sites test `Task.isCancelled` instead. **Watch the cleanup when you add the guard:** an early return skips a trailing `isLoading = false` and strands the spinner for good — it must be in a `defer`.
