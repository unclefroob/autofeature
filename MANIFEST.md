...
| `.claude/commands/market-review.md` | `CUSTOM` | Standalone market & fundability review. Captures the product/idea from context (reads the repo README to ground it), runs the market-review Workflow (frame → market ∥ gap ∥ VC → bear case → synthesize) with live cited web research, and produces an investment memo answering is-it-useful / market-gap / can-I-get-funding. Saves to `.autofeature/market-review-[date].md`. Advisory only. |
