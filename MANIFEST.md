...
| `.claude/commands/award-map.md` | `CUSTOM` | Maps one Australian modern award into `rosterio-compliance-service`. NOT the feature pipeline — it borrows only branch/review/ship at the end, because the work is reading an award. **Enumeration-first ordering** (preflight → acquire text → transcribe TOC + sub-clause counts from the Commission's consolidated PDF → read the extract's shape → expressiveness triage → stand up the eight closure axes → author against the resulting FIXED work list → scenario suite → **semantic verification (Step 7.5)** → closure run + review pack → ship). THREE hard checkpoints that stop even in `mode:automated`: the transcription, the triage, and the human sign-off on the closure + verification report before ship. Five Iron Laws, plus two Prohibitions added for verification (no shipping past a high-confidence semantic defect unresolved; never present a green run as a compliance guarantee). Reads `awards/*.md`. |
| `.claude/commands/award-verify.md` | `CUSTOM` | Standalone, re-runnable semantic verification for an already-mapped award. Queries every predicate-bearing `rule_*` row (with `clause_text`) into one JSON array, then invokes the `awards/verify-workflow.md` Workflow script against it. Supports `tables:`/`clauses:` filters so a wage-review or award-variation only needs to re-check the rows it actually touched, not the whole award. Called by `award-map.md` Step 7.5, and usable on its own afterward — this is the piece that answers "was a rule read correctly," which coverage/closure structurally cannot. |
| `.claude/commands/award-drift.md` | `CUSTOM` | Manual, on-demand check for whether a mapped award's own wording has moved since it was last checked — an award variation, not a rate change (rates already flow through free on the next `db:load`). Re-fetches the award text, diffs clause-by-clause against the previously stored version, names exactly which `rule_*` rows are affected, and points at `award-verify ... clauses:` scoped to just those. Triggered by a human who suspects a variation landed, not a cron — closure and verification are both point-in-time, and nothing else re-checks the award's text against the live source after mapping. |
...
| `awards/verify-workflow.md` | `CUSTOM` | **Semantic verification** — the Workflow script closure can't provide. Two agents independently re-derive every predicate-bearing row's structured fields (`rule_condition`, `rule_span`, `rule_overtime_threshold`, `rule_junior_band`, `rule_allowance`, `rule_roster`) from `clause_text` alone, blind to the shipped row and to each other; a plain-code diff flags disagreement; a third agent argues the shipped row is wrong only on flagged rows. Read by `.claude/commands/award-verify.md` and invoked as Step 7.5 of `award-map.md`. Ends with an explicit ceiling: raises confidence, doesn't retire the risk — correlated blind spots across independent agents survive it, which is why a `high`-confidence defect blocks and a clean run still isn't a guarantee. |
