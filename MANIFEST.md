...
| `adapted/browser-session.md` | *(none)* | `CUSTOM` | **Session establishment for browser-driven testing — how to authenticate WITHOUT typing a password.** Claude in Chrome refuses password fields, so a login step built on `form_input` into `#password` blocks every auth-gated flow behind it; this replaces it. Three refuse-the-run preconditions (non-production target, fixture credentials only, never a real person's password), a detection procedure that reads the repo's storage key / cookie flags / seeder and probes the live login endpoint rather than guessing, and a six-rung ladder: **0** seeder emits sessions · **1** dev-only `/__test/login` endpoint (404-guarded, reuses the app's own `issueSession`) · **2** same-origin `fetch` login so an httpOnly `Set-Cookie` lands in the real jar · **3** token injected at the frontend's actual storage key · **4** storage-state replay · **5** the human's own session (the only rung for real credentials). Two-signal verification gate — the API must name the intended user AND the DOM must show a marker that cannot render anonymously — because a session that silently didn't take makes every flow fail for the same wrong reason and reads as a catastrophically broken product. Multi-role handling, redaction + gitignore rules for session blobs, and a defined return contract for callers. Shared by `ui-test.md`, `test.md` and `award-ui.md` |
...
| `.claude/commands/do.md` | `CUSTOM` | Plain-words entry point — runs `orchestrator/task-router.md` on any task and dispatches (pipeline, another command, or a read-only specialist survey). `route-only` prints the Route Card without running anything. |
...
| `.claude/commands/ui-test.md` | `CUSTOM` | **Browser-only acceptance driver for self-seeded test data.** Web only and deliberately — simulators are `test.md`'s job. Seeds the fixture users, mints a session per role via `adapted/browser-session.md` (no password ever enters the browser's input path), verifies identity two ways before driving, then derives its own plan and walks each role's flows in Claude-in-Chrome. Keeps one deliberately-BLOCKED flow that opens the real login screen, so "login is never tested" cannot hide inside a green run. Caches the resolved auth profile to `.autofeature/session-profile.json`. Reports per-flow PASS/FAIL/BLOCKED with screenshots + console/network errors; hands failures to `/autofeature`, never edits app code |
| `.claude/commands/copy.md` | `CUSTOM` | Copy audit + fix + write command. Audit mode: sonnet surface map → parallel sonnet auditors per surface bucket, findings (file:line · tells · severity · suggested rewrite) saved to `.autofeature/copy-audit-[date].md`. Fix mode: micro-tier in-place string rewrites (opus, edits string literals/JSX text only, preserves interpolation tokens and matched-on strings), verify via re-grep + typecheck, leaves diff uncommitted. Write mode: drafts 2–3 voice-consistent options per string. Reads `adapted/feature-copy-audit.md` + optional `.autofeature/voice.md`. |
| `.claude/commands/backend-api-skills.md` | `CUSTOM` | **The backend-shape command.** One entry point for getting a backend API into the correct shape: `audit` (read-only violation report, V1–V14, ranked by risk), `create` (greenfield skeleton via `backend/scaffold.md` Blueprint A, or one correctly-shaped domain slice via Blueprint B), `fix` (behaviour-preserving refactor — the pipeline `api-standards` used to own, generalised past one repo). Stack-adaptive: detects framework/ORM/validator/test-runner per `backend/stack-profiles.md` and maps the four layers onto it, rather than assuming Express+Mongoose. Iron Law in `fix`: green baseline before AND identical-green after — a pre-existing route test that had to be edited means behaviour changed. Reads `backend/*.md`, plus `ritchies/conventions.md` when the target is a Ritchies repo and the repo's own `.autofeature/patterns.md` when present (canon outranks the standard). |
| `.claude/commands/api-standards.md` | `CUSTOM` | **Alias — superseded by `backend-api-skills`.** Forwards there with the Ritchies profile pre-selected, mapping the old grammar (`audit` → `audit`, `<domain>` → `fix <domain>`, `all` → `fix all`, bare → audit-then-offer). Kept so existing muscle memory and scripted invocations keep working. Previously held the refactor pipeline itself; that now lives in `backend-api-skills` fix mode. |
...
| `.claude/commands/ritchies.md` | `CUSTOM` | Ritchies-platform wrapper over `autofeature.md`. Hard-wires the four repos (`ritchies-platform-api`/`-web`, `ritchies-mobile`, `ritchies-android`) instead of the generic cross-repo-detect; always scaffolds the API first + freezes the contract via `api-contract-broker`, then asks which clients (web/iOS/Android) to fan out. Loads `ritchies/conventions.md`, enforces each repo's deliberate conventions (no Retrofit/Koin on Android, no react-query on web, iOS-parity mobile, four-pillar RBAC, employee-number auth). Overrides base Steps 4/5b/6/7b/8 only; everything else runs per autofeature.md. **Step 6 now carries the per-repo deploy-trigger table:** merging `main` auto-deploys the API and web, does nothing on Android, and starts an **App Store Connect build** on iOS — so iOS ships on its branch with the PR left open, and merging is a release decision the user makes. Step 8's iOS verification delegates to `.claude/commands/ritchies-ios-test.md`. |
| `.claude/commands/ritchies-ios-test.md` | `CUSTOM` | **iOS build + simulator verification for a machine with no Xcode.** `ritchies-mobile` cannot be compiled on the Linux box the pipeline runs from, so iOS verification is a hand-off to a Claude session on the Mac, and this is the protocol for doing that reliably: push first and name the exact SHA in every message; pick the Mac **by machine name, never by project name** (a project-named session is not hardware — this already cost four unanswered messages and a wrong report to the user); demand the compile result on its own before any fixing or testing; name the risks already checked against the house pattern so a helpful session doesn't 'fix' working code off-pattern; flag undeployed endpoints, which 404 against dev. Drives the simulator through `simctl` (device lifecycle, deep links, permissions, log stream) and **XCUITest** (UI taps and assertions by accessibility identifier) — **never by controlling the screen**: pixel-clicking fails silently when it lands on the wrong element, so a mis-driven pass reads exactly like a real one, and screenshots are evidence rather than the control mechanism. An element that cannot be addressed by name is a finding — add an accessibility identifier instead of falling back to coordinates. Ends on one of four verdicts — built+simulator-verified / built+tests-green / compile-checked-only / never-compiled — bound to the SHA it applies to. Encodes the traps: cross-session delivery fails silently in BOTH directions (never write down a peer result you have not received), a test filter matching the FILENAME can match zero tests and read as zero failures, `swiftc -typecheck` is blind to the project file, and 401-vs-404 is the cheapest deploy probe. |
| `.claude/commands/patterns.md` | `CUSTOM` | Coding-canon command. Audit mode: sonnet surface map → 4 parallel sonnet census groups (structure/handlers · validation/auth + helper hunt · errors/logging + dead scaffolding · data access/dates), report to `.autofeature/patterns-audit-[date].md` + DRAFT `.autofeature/patterns.md`. Establish mode (always interactive): resolve `⚖ PROPOSED` decisions, activate canon, wire enforcement (eslint ratchet / husky pre-push / CI offer / dead-scaffolding removal). Check mode: diff-scoped conformance (also runs at pipeline Step 9b2). Fix mode: hands one drift area to `/autofeature` with `[skip-product-review]`. Reads `adapted/feature-patterns-audit.md`. |
...
The portable backend API standard, extracted from the Ritchies API conventions and generalised so it applies to any backend. Read by `.claude/commands/backend-api-skills.md` in every mode.

| File | Status | Notes |
|------|--------|-------|
| `backend/api-standard.md` | `CUSTOM` | The authoritative standard, stated stack-neutrally. Iron Law (business logic lives in a layer that does not know HTTP exists) → the four layer contracts as explicit MAY / MUST NOT lists → reuse-before-extract and shared-helper extraction → cross-cutting contracts (central error funnel, boundary validation, **route-level guards vs resource-scoped checks**, server-authored verdicts) → the two-tier test contract → the **V1–V14 violation catalogue** with per-stack tells and 🔴/🟡/🟢 severity → the verify gates including the identical-green rule. Closes with what it deliberately does NOT dictate (directory names, framework, repository layer, REST shape) so it can't be misread as a style guide. |
| `backend/stack-profiles.md` | `CUSTOM` | Detection-before-generation. A runnable detection block (framework / data layer / validator / test runner / existing layering), the rule that ambiguity STOPS the run, layer-mapping tables for Express·Fastify·NestJS·Koa × Mongoose·Prisma·TypeORM·Knex (including the per-ORM V1 tell and the Nest DI exception to the function-module rule), command discovery where **CI outranks `package.json` scripts**, a thinner-guidance path for non-Node backends, and the authority order: repo `patterns.md` > the standard > what existing files happen to do. |
| `backend/scaffold.md` | `CUSTOM` | The two `create` blueprints. **A: greenfield skeleton** — app/entrypoint split (route tests depend on it), central error handler registered last, typed domain errors, the auth seam and single access resolver, central config, health + readiness, a test harness proven green in both tiers, the four gate commands, then the first domain slice; plus an explicit NOT-in-the-spine list (logging framework, rate limiting, OpenAPI, containerisation, CI) so a scaffold doesn't become a framework nobody chose. **B: domain slice** — files in dependency order, the reuse pass that runs *before* generation, and a definition of done that includes registering capability keys (a capability that exists only in code is a deploy-time outage). |
...
| File | Status | Description |
|------|--------|-------------|
| `orchestrator/task-router.md` | `CUSTOM` | Triage before the pipeline — classifies a free-form request's intent (build/fix/investigate/audit/test/review/award), resolves the target repo by project signature (asks when ambiguous), routes to an existing command or a read-only **specialist survey** (the stack's architect, `Mode: survey`, with surface checklists for styles/navigation/networking/state/a11y/perf), assigns the model per model-tiers.md, and emits a Route Card. Used by `autofeature.md` Step 0.75 and `do.md` |
...
| `orchestrator/model-tiers.md` | `CUSTOM` | Model-tier (cost/quality) policy — pins each spawned agent / Workflow phase to sonnet (floor) or opus so the fan-out doesn't all run on the session model. Base tiers + escalation rules (E1 cross-repo, E2 high-risk surface, E3 destructive data change, E4 unclear design) + failure escalation (2nd retry on opus), recorded per run as a Model Plan in the Feature Brief. Profiles economy/balanced/quality + per-run `model:` override. Referenced by every command that fans out. |
...
| Model-tier cost control | `orchestrator/model-tiers.md` + `model:` pins at every spawn site (`autofeature.md`, `feature-advice.md`, `feature-product-review.md`, `market-review.md`, `scope.md`, `product-review.md`, `seo.md`, `feature-test.md`, `feature-deploy-verify.md`, `skill-wiring.md`) | Stops the whole fan-out inheriting an expensive session model. Each `Agent()`/Workflow `agent()` carries an explicit tier (floor Sonnet; fixed Opus for the bear-case gate, market memo, copy rewrites). `/autofeature` raises specific tasks to Opus per run via a Model Plan written after the scope gate (cross-repo, high-risk surface, destructive migration, unclear design, or repeated failure). One editable policy file; per-run `model:` override |
