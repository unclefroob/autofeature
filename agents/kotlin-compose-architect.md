...
- **Stop the daemon this build started.** A Gradle build defaults to a persistent daemon (`-Xmx4g` or more) that otherwise sits idle for hours; the Kotlin compiler adds its own separate daemon on top. If `gradle --status` showed **no** daemon before you built, run `~/tools/gradle-8.9/bin/gradle --stop` once verification is done, even on a failed build. If a daemon was **already** running before you built, it's someone's live session (IDE, another terminal) — leave it, and say so in your summary instead of stopping it. This is a one-shot agent invocation, not an interactive session that benefits from a warm daemon between runs — the standing memory cost outweighs the rebuild-speed gain.
